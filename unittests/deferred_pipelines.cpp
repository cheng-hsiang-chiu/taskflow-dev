        ...
        if (j == N) {
          ...
          if (pf.token() > 0) {
            switch(pf.deferred()) {
              ...
              default:
                printf("stage 1: Scheduling token %zu is scheduled for token %zu.\n", pf.token()-1, pf.token());
            }
          }
        }
...
void pipeline_1P_S_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    tf::Pipeline pl(
      L,
      tf::Pipe{tf::PipeType::SERIAL, [N, &source, &j, &mybuffer, L](auto& pf) mutable {
        if(j == N) {
          pf.stop();
          return;
        }
...
void pipeline_2P_SS_DeferPreviousToken(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    tf::Pipeline pl(
      L,
      tf::Pipe{tf::PipeType::SERIAL, [N, &source, &j1, &mybuffer, L, &collection1](auto& pf) mutable {
        if(pf.token() == N) {
          pf.stop();
          return;
        }
        else {
          printf("Token %zu on line %zu\n");
          switch(pf.deferred()) {
            case 0:
              if (pf.token() > 0) {
                pf.defer(pf.token()-1);
      ...
      tf::Pipe{tf::PipeType::SERIAL, [N, &source, &j2, &mybuffer, L, &collection2, &mutex](auto& pf) mutable {
        //REQUIRE(j2 < N);
        REQUIRE(pf.token() % L == pf.line());
        {
          std::scoped_lock<std::mutex> lock(mutex);
        ...
        //REQUIRE(pf.token() == mybuffer[pf.line()][pf.pipe() - 1]);
        //REQUIRE(source[j2] + 1 == *(pf.input()));
        //j2++;
      }}
    );
    ...
    auto test = taskflow.emplace([&](){
      ...
      for (size_t i = 0; i < source.size(); ++i) {
        REQUIRE(collection1[i] == source[i]);
        REQUIRE(collection2[i] == source[i]);
      }
